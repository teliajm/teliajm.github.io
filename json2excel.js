...
const recordingBaseUrl =
  'https://tools-k8s.ace.teliacompany.net/voice/convhub/monitor/recordings/';

/* Initialize "events choice" if not set already, default to include */
if (localStorage.getItem('j2e_includeEvents') === null)
  localStorage.setItem('j2e_includeEvents', 'checked');
...
const originalXMLHttpRequest_setRequestHeader =
...
XMLHttpRequest.prototype.open = function (method, url, async, user, password) {
  if (!url.includes('/history/v1/dialogs'))
...
async function insertDownloadButton(dialogsCount) {
  ...
  const searchButton = document.querySelector(
    '.ca-config-table-container .ca-search-button:not(.download)'
  );
  searchButton.insertAdjacentHTML(
    'afterEnd',
    `
        <button id="download-button" style="margin-left:10px" type="button" class="download v-btn v-btn--is-elevated v-btn--has-bg theme--light v-size--small primary">
        ...
        <input ${localStorage.getItem(
          'j2e_includeEvents'
        )} onclick="toggleIncludeEvents(event)" style="margin-left: 10px" type="checkbox" id="includeEvents"> 
...
async function transformJsonToExcel(json, extension, startDate, untilDate) {
    ...
    for (var session in dialog.sessions) {
      ...
      for (var event in session.dialog_events) {
        ...
        if (event?.type == 'INPUT') {
          ...
          utterenceRecordingId =
            event?.payload?.input?.speechRecognitionInput?.utterenceRecordingId;
          if (event?.payload?.input?.eventInput?.name) {
            ...
            eventName = '';
            transcript =
              event?.payload?.input?.speechRecognitionInput
                ?.recognitionResults[0]?.transcription?.transcript;
            userInput = true;
          }

          if (shortTimestamp && timestamp && timestamp.indexOf('.') !== -1)
          ...
          if (transcript.length > transcript_len)
        ...
        if (event?.type == 'OUTPUT') {
          if (event?.payload?.output?.responses) {
            response =
              event?.payload?.output?.responses[0]?.directAudioResponse
                ?.transcript;
            if (response) response = response.replace(regex, '');
          } else response = '';

          parameters =
            event?.payload?.output?.action?.transferOperatorAction?.parameters;
  ...
  Object.keys(worksheet)
    .filter((k) => k[0] === 'K')
    .filter((v) => worksheet[v].v)
    .forEach((c) => {
      worksheet[c].l = {
        Target: `${recordingBaseUrl}${worksheet[c].v}`,
        Tooltip: `${recordingBaseUrl}${worksheet[c].v}`,
      };
    });

  /* Set filename to (first) customer name if no extension */
  if ((!extension || extension === '') && rows.length > 0 && rows[0]?.Customer)
    extension = rows[0]?.Customer + '_';
